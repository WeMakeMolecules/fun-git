...
#watch out for different Augustus versions,  I used 3.4.0, different verions may have different comments, in such case update the following lines to get clean aminoacids
...
$line=~s/.+\-\-gff3\=on \-\-stopCodonExcludedFromCDS\=off//g;
	...
	$line=~s/start gene g/fig|$rand1.$rand2.peg./g;
	$line=~s/.+end.+/\n/g;
	$line=~s/\]//g;
	$line=~s/ //g;
	print AMINO $line;
...
system  "grep \-E \'/ID\=.+.cds\"\|\/description\' $ARGV[1]\| tr \'\\n\' \' \'\|sed \'s/\\/ID\/\\n/g\'\|sed \'s\/  \/\/g\' \> raw_grepped_$rand1.$rand2";
open FILEGREP, "raw_grepped_$rand1.$rand2";
while($line=<FILEGREP>){
	if ($line=~/\s+\n/){$dummy="1";next;}
	if ($line=~/="g\d+.t1.cds"\/description\=\".+/){
	$line=~/\="(g\d+)(.t1.cds"\/description\=\")(.+)/;
	$id="$1";
	$annotation="$3";
...
system "grep transcript $ARGV[0]|grep '#' -v>transcript_list_$rand1.$rand2";
...
open COORDINATES, "transcript_list_$rand1.$rand2" or die "I cannot open the funtions table file\n";
while ($line=<COORDINATES>){
	...
	open FUNCTIONS, "functions_table_$rand1.$rand2" or die "i cant see the FUNCTIONS file \n";
		while ($id=<FUNCTIONS>){
			...
			$match="$1";
			$function="$2";
			$match=~s/g/fig|$rand1.$rand2.peg./g;
			if ($match eq $gene){
...
close COORDINATES;
close TABLE;
system "rm transcript_list_$rand1.$rand2 raw_grepped_$rand1.$rand2 amino_acids_$rand1.$rand2 functions_table_$rand1.$rand2";
;;
