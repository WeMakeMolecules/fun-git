...
with open(GBK_file,'r',encoding = 'utf-8') as file:
    ...
    for i,_ in enumerate(lines):
        if lines[i].startswith("                     /gene="):
            ...
            function_dict[gene] = ['unknown function']
        elif lines[i].startswith("                     /description=") and genes_found==True:
            genes_found=False
            function=lines[i].strip().split("=")[-1].replace('"', '')
            function_dict[gene] = [function]
        elif lines[i].startswith("                     /description=") and genes_found==False:
            function=lines[i].strip().split("=")[-1].replace('"', '')
            function_dict[gene].append(function)
        else:
            pass
...
df['function']=  [" ".join(function) for function in function_dict.values()]
