...
awk 'BEGIN {FS="\t"}; {print "mv,"$1"*genomic.fna,"$8$9$1".fna"}' list.txt | sed s'/ /_/g'|sed s'/=/-/g'|sed s'/strain//'|sed s'/(//g'|sed s'/)//g'|sed 's/-/_/g'|sed 's/,//g'|sed 's/://g'|sed 's/\///g'|sed 's/,/ /g' >rename.sh
